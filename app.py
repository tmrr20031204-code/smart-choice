...
・「確認できない」と「存在しない」を厳格に区別してください。知識ベースに完全一致する型番がない場合であっても、「国内未発表」「中国向け」「存在しない」「並行輸入」「正規サポート対象外」などと根拠なく推測・断定し、不当にユーザーの不安を煽ることを【厳禁】とします。
...
・「危険！要注意！」は、相場より50%以上法外に高い明らかな過剰請求や悪質ぼったくり、または明確な詐欺リスクがある場合のみに厳格に限定し、適正〜割安な良品（例: 7万円台のフラッグシップ機等）を誤って「危険！」と判定してはならない。
...
async def analyze_images(
    ...
    try:
        ...
        if category == "車検・整備":
            ...
            category_instruction = "【分析対象: スマホ・PC・周辺機器】CPU/SoC世代（Apple Mシリーズ、Intel Core Ultra等）、メモリ容量、SSD容量、画面仕様から市場相場を精密に推計。高機能フラッグシップ機（MacBook Proやハイスペック機）に型落ちローエンド相場を誤認適用しないこと。新品購入時の不要な高額初期設定サポート（1万〜3万円）や不要オプションを見抜いてください。パソコン本体や周辺機器の見積もりの場合は、クリーンな 'search_keyword'（メーカー名+型番）を設定してください。※'recommend_ec_search' は false に設定してください。"
        elif category == "不用品買取":
