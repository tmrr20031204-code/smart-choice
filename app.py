...
【ハルシネーション・不当な決めつけの徹底排除】
・「中国市場向け」「並行輸入」「正規サポート対象外」などと根拠なく推測・断定し、不当にユーザーの不安を煽ることを【厳禁】とします。
・製品のグレード（上位フラッグシップ／中位／下位エントリー）を的確に見極め、高機能上位機に対して下位機種の格安相場（例: 全自動ロボット掃除機に4〜5万円を当てはめる等）を誤認適用する重大誤診を絶対に犯さないこと。
・「危険！要注意！」は、相場より50%以上法外に高い明らかな過剰請求や悪質ぼったくり、または明確な詐欺リスクがある場合のみに厳格に限定し、適正〜割安な良品（例: 7万円台のフラッグシップ機等）を誤って「危険！」と判定してはならない。

【プロの価格監査原則】
1. 最新かつ正確な市場知識を論理的に駆使し、事実に基づいた客観的評価を行う。
...
async def analyze_images(
    ...
    try:
        ...
        if category == "車検・整備":
            ...
            category_instruction = "【分析対象: 家電（ロボット掃除機・冷蔵庫・洗濯機等）】機能・スペックに応じた業界標準の実勢相場を正しく適用してください。\n・全自動フラッグシップ（ロボット掃除機のOMNI/全自動ドック、ゴミ収集＋モップ温水洗浄＋熱風乾燥付き等）: 国内実勢相場は『約9万円〜18万円』（定価15万〜25万円クラス）。もし提示価格が7万〜9万円程度であれば非常に『お買い得』または『割安』であり、【絶対に4万〜5万円のエントリー機相場と誤認・混同してはならない】。\n・ミドルクラス（自動ゴミ収集ステーションのみ、簡易水拭き）: 実勢相場『約4.5万円〜8万円』\n・エントリークラス（ステーションなし単体機）: 実勢相場『約2万円〜4.5万円』\n※型番が最新・派生型番であっても『中国向け』『国内正規サポート外』等と根拠なく決めつけて危険扱いしないこと。EC最安値比較用のクリーンな 'search_keyword'（メーカー名+シリーズ名）を出力してください。※'recommend_ec_search' は false に設定してください。"
        elif category == "エアコン":
