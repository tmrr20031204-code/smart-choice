...
async def analyze_images(
    ...
    try:
        ...
        if category == "車検・整備":
            ...
            category_instruction = "【分析対象: 家電（冷蔵庫・洗濯機・掃除機など）】本体価格や搬入経路（階段昇降費用、クレーン吊り上げ等）、設置費用、リサイクル料金が適正相場か確認してください。\n※【重要: システム連携情報】現在連携している専用業者は「Yahoo!ショッピング（総合EC）」です。家電本体や部品の購入に幅広く対応しているため、いかなる場合も必ずJSONの 'recommend_ec_search' を false に設定してください（trueにすると不具合になります）。"
        elif category == "エアコン":
            ...
            category_instruction = "【分析対象: 火災保険】不要な特約や、重複している補償内容がないか確認してください。\n※【重要: システム連携情報】現在連携している専用業者は「火災保険の一括見積もりサービス」です。もしユーザーが自動車保険や生命保険の見積もりを誤って送信してきた場合は、専用業者の対象外となるため、必ずJSONの 'recommend_ec_search' を true に設定してください。"
